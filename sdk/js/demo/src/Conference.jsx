...
class Conference extends React.Component {
  ...
  _onChangBigVideo = (data) => {
    let id = data.id;
    let index = data.index;
    console.log("changeBigVideo id:" + id + "  index:" +  index);
            ...
            <div style={{ whiteSpace: "nowrap", overflow: "scroll" }}>
              {streams.map((item, index) => {
                return index > 0 ? (
                  ...
                  >
                    <SmallVideoView id={item.id} stream={item.stream} index={index} onClick={this._onChangBigVideo} />
                  </div>
                ) : (
                  ""
                );
